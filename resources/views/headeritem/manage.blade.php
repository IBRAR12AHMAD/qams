    ...
    <div id="kt_app_content_container" class="app-container container-xxl ">
        ...
        <div class="card card-flush">
            <div class="card-header align-items-center py-5 gap-1 gap-md-5">
                ...
                <a type="button" class="btn btn-sm btn-light-primary" href="{{ route('addheaderitem') }}">
                    <i class="ki-duotone ki-plus fs-2"></i>Add Header Item
                </a>
            </div>
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
                    ...
                    <tbody>
                        {{-- @foreach($itemheaders as $itemheader)
                            <tr>
                                <td>{{ $itemheader->id }}</td>
                                <td>{{ $itemheader->title }}</td>
                                <td>{{ $itemheader->order_number }}</td>
                                <td>
                                    @if ($itemheader->status === 1)
                                ...
                                <td>
                                    @if ($edit_permission == 'true' || $check_all_permission == 'true')
                                        <a href="{{ route('editheaderitem', $itemheader->id) }}" class="btn btn-sm btn-success">Edit</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach --}}
...
<script>
    $(function () {
        ...
        var table = $('#kt_ecommerce_report_views_table').DataTable({
            ...
            buttons: [
                { extend: 'copyHtml5', title: 'Itemheader', className: 'd-none' },
                { extend: 'excelHtml5', title: 'Itemheader', className: 'd-none' },
                { extend: 'csvHtml5', title: 'Itemheader', className: 'd-none' },
                { extend: 'pdfHtml5', title: 'Itemheader', className: 'd-none' }
            ]
        });
