        ...
        <div class="card mb-5 mb-xl-12">
            <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse" data-bs-target="#kt_account_profile_details" aria-expanded="true" aria-controls="kt_account_profile_details">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">Create Campus & Department</h3>
                </div>
            </div>
            <div id="kt_account_settings_profile_details" class="collapse show">
                <form id="kt_account_profile_details_form" action="{{ route('storecampusdepartment') }}" method="POST">
                    @csrf
                            ...
                            <div class="col-lg-12">
                                <div class="row">
                                    <div class="col-lg-6 fv-row">
                                        ...
                                        <select name="campus_id" class="form-select form-select-sm fw-semibold mb-3 mb-lg-0" data-control="select2"
                                            oninvalid="this.setCustomValidity('Please Select Campus')" oninput="this.setCustomValidity('')" required>
                                            <option value="" disabled selected>- Select Campus -</option>
                                            @foreach ($campuses as $campus)
                                                <option value="{{ $campus->id }}"
                                                    @if(collect(old('campus_id'))->contains($campus->id)) selected @endif>
                                                    {{ $campus->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-lg-6 fv-row">
                                        <label class="col-lg-4 col-form-label required fw-semibold fs-6">Deparment</label>
                                        <select name="department_id[]" class="form-control form-control-sm fw-semibold mb-3 mb-lg-0 @error('department_id') is-invalid @enderror" data-control="select2" multiple>
                                            @foreach ($deparments as $deparment)
                                                <option value="{{ $deparment->id }}"
                                                    @if(collect(old('department_id'))->contains($deparment->id)) selected @endif>
                            ...
                            <div class="col-lg-12">
                                <div class="row">
                                        ...
                                        <select name="program_id[]" class="form-control form-control-sm" data-control="select2" multiple>
                                            @foreach ($programs as $programs)
                                                <option value="{{ $programs->id }}"
                                                    @if(collect(old('program_id'))->contains($programs->id)) selected @endif>
                                    ...
                                    <div class="col-lg-6 fv-row">
                                        ...
                                        <select name="status" data-control="select2" class="form-select form-select-sm fw-semibold mb-3 mb-lg-0 @error('status') is-invalid @enderror" value="{{ old('role_id') }}">
