    ...
    <div id="kt_app_content_container" class="app-container container-xxl ">
        ...
        <div class="card card-flush">
            <div class="card-header align-items-center py-5 gap-1 gap-md-5">
                ...
                <a type="button" class="btn btn-sm btn-light-primary" href="{{ route('addcampusdepartment') }}">
                    <i class="ki-duotone ki-plus fs-2"></i>Add Campus Department
                </a>
            </div>
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
                    ...
                    <tbody>
                        {{-- @foreach($campusDepartments as $campusdepartment)
                            <tr>
                                <td>{{ $campusdepartment->id }}</td>
                                <td>{{ $campusdepartment->campus->name }}</td>
                                <td>
                                    @if ($campusdepartment->status === 1)
                                ...
                                <td>
                                    @if ($edit_permission == 'true' || $check_all_permission == 'true')
                                        <a href="{{ route('editcampusdepartment', $campusdepartment->id) }}" class="btn btn-sm btn-success">Edit</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach --}}
...
<script>
    $(function () {
        ...
        var table = $('#kt_ecommerce_report_views_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('managecampusdepartment') }}",
                data: function(d) {
                    $('#kt_ecommerce_report_views_table tfoot th').each(function(i) {
                        var val = $(this).find('input').val();
                        if (val) d.columns[i].search.value = val;
                    });
                }
            },
            ordering: false,
            ...
            buttons: [
                { extend: 'copyHtml5', title: 'Campus Department', className: 'd-none' },
                { extend: 'excelHtml5', title: 'Campus Department', className: 'd-none' },
                { extend: 'csvHtml5', title: 'Campus Department', className: 'd-none' },
                { extend: 'pdfHtml5', title: 'Campus Department', className: 'd-none' }
            ]
        });
