        ...
        <div class="card mb-5 mb-xl-12">
            <div class="card-header border-0 cursor-pointer" role="button">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">Edit Checklist</h3>
                </div>
            </div>
            <div id="kt_account_settings_profile_details" class="collapse show">
                <form id="kt_account_profile_details_form" action="{{ route('updatechecklist') }}" method="POST">
                    @csrf
                    <input type="hidden" name="id" value="{{ $checklists->id }}">
                                ...
                                <div class="row">
                                    <div class="col-lg-6 fv-row">
                                        ...
                                        <input id="title" type="text" class="form-control form-control-sm mb-3 mb-lg-0" name="title"
                                            value="{{ old('title', $checklists->title) }}" placeholder="Enter Checklist Title" required>
                                    ...
                                    <div class="col-lg-6 fv-row">
                                        ...
                                        <select name="status" data-control="select2"
                                            ...
                                            <option value="1" {{ old('status', $checklists->status) == '1' ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ old('status', $checklists->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                        <span class="text-danger">{{ $errors->first('status') }}</span>
                                    </div>
                                    {{-- <div class="col-lg-6 fv-row">
                                        <label class="col-lg-4 col-form-label required fw-semibold fs-6">Audit</label>
                                        <select name="audit_id" data-control="select2" class="form-select form-select-sm fw-semibold">
                                            <option disabled selected>- Select Audit -</option>
                                            @foreach ($audits as $audit)
                                                <option value="{{ $audit->id }}"
                                                    {{ old('audit_id', $checklists->audit_id) == $audit->id ? 'selected' : '' }}>
                                                    {{ $audit->title }}
                                                </option>
                                            @endforeach
                                        </select>
